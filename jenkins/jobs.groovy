...
projects.each {
    ...
    mavenJob(projectName + '_master-deploy') {
        ...
        if (projectName.equals('properties') || projectName.equals('pessimistic-mongodb') || projectName.equals('javassist-classpath-scanner')) {
            label('maven')
        } else {
            label('maven-old')
        }
        ...
        configure { project ->
            project / publishers << 'hudson.plugins.sonar.SonarPublisher' {
                ...
                branch()
                language()
                mavenOpts("-Xmx1024m -Xms256m -Xdoclint:none")
